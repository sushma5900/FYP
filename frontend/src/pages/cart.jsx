...
async function getProductInfo(productId, size) {
  ...
  let product = null;
  try {
    const res = await axios.get(`http://localhost:3120/api/products/find/${productId}`);
    ...
    <>
      ...
      <Row
        style={{
          ...
          borderBottom: "0.0625rem solid #c0c0c0",
        }}
      >
        <Button variant="none" onClick={() => navigate(-1)}>
          <ArrowLeft size={"2rem"} />
        </Button>
        <h3 style={{ fontFamily: "Poppins" }}>SHOPPING CART</h3>
        <Cart2 size={"2rem"} />
      </Row>
      <Container>
        {items.length ? (
          ...
          >
            ...
            <Col>
              ...
              >
                ...
                <Button style={{ padding: "1rem" }} onClick={checkoutOperator}>
                  Checkout
                </Button>
              </div>
            </Col>
